...
class TemplateController extends AdminController {
    ...
    function _initialize() {
        parent::_initialize();
        if(LANG_ID == 2){
            $this->filepath = TMPL_PATH.'En/'.$this->sysConfig['DEFAULT_THEME'].'/';
            $this->publicpath = TMPL_PATH.'En/'.$this->sysConfig['DEFAULT_THEME'].'/public/';
        }else{
        ...
        if(isset($_GET['iswap'])){
            $this->filepath = TMPL_PATH.'Mobile/'.$this->sysConfig['DEFAULT_THEME'].'/';
            $this->publicpath = TMPL_PATH.'Mobile/'.$this->sysConfig['DEFAULT_THEME'].'/public/';
        }

        $this->assign('nav1',1);
        if(!empty($_GET['type']) && $_GET['type'] == 'css')
            $this->assign('nav2',8);
        else
            $this->assign('nav2',7);
    }
