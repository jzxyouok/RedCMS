...
function geturl($cat,$data='',$Urlrule=''){
  ...
  if($cat['ishtml']){
    ...
    if($URL_MODEL==0){
      ...
      if($id){
        $url[] = U("home/$cat[module]/show?id=$catid&chid=$id".$langurl);
        $url[] = U("home/$cat[module]/show?id=$catid&chid=".$id.$langurl.'&'.C('VAR_PAGE').'={$page}');
      }else{
...
function teb_cn_en($cnid,$enid){
  return LANG_NAME=='cn'?$cnid:$enid;
}
...
function toimages($str){
  ...
  foreach($images as $info){
    $in=explode('|',$info);
    $s.='<li><a href="/index.php?a=index&m=page&id=24"><img width="155" height="116" src="'.$in[0].'" /></a></li>';
  }
  return $s;
}
