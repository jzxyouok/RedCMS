...
class Gr extends TagLib{
  ...
  public function _flash($tag,$content) {
    ...
    //下面拼接输出语句
    $parsestr  = '';
    $parsestr .= '<?php  $_result='.$sql.';$count=count($_result);if ($_result): $'.$key.'=0;';
    $parsestr .= 'foreach($_result as $key=>$'.$id.'):';
  ...
  public function _nav($tag,$content) {
    ...
    if(is_array($category_arr)){
      ...
      import ( '@.ORG.Tree' );
      $tree = new Tree ($array);
      $parsestr = $tree->get_navs($catid,$level);
    }
  ...
  public function _subcat($tag,$content) {
    ...
    if($self){
      $ifleft = '('; $selfcondition = ') or (  $'.$id.'[\'ismenu\']==1 && intval('.$catid.')==$'.$id.'["id"])';
    }
    $count       =   $this->autoBuildVar($count);
    $key       =   $this->autoBuildVar($key);
    $parsestr  = '';
    $parsestr .= '<?php '.$count.'=0;';
    $parsestr .= 'foreach($Cats as $keyy=>$vy):';
    $parsestr .= 'if($vy["ismenu"]==1&&intval('.$catid.')==$vy["parentid"]):';
    $parsestr .= $count.'++;?>';
    $parsestr .= '<?php endif;?>';
    $parsestr .= '<?php endforeach;?>';
    $parsestr .= '<?php '.$key.'=0;';
    $parsestr .= 'foreach($Cats as $key=>$'.$id.'):';
  ...
  public function _catpos($attr) {
    ...
    if(is_numeric($tag['catid'])){
      ...
      //下面拼接输出语句
      $parsestr  = '';
      $parsestr .= '<?php $ci=0;  $arrparentid = array_filter(explode(\',\', $Cats[$'.$tag['catid'].'][\'arrparentid\'].\',\'.$'.$tag['catid'].'));';
      $parsestr .= 'foreach($arrparentid as $cid):';
      $parsestr .= '$ci=$ci+1;if(count($arrparentid)==$ci):$class="bc"; endif; $parsestr[] = \'<a class="\'.$class.\'" href="\'.$Cats[$cid][\'url\'].\'">\'.$Cats[$cid][\'catname\'].\'</a>\';?>';
  ...
  public function _kefu($tag,$content) {
    ...
    $id = !empty($tag['id'])?$tag['id']:'r';  //定义数据查询的结果存放变量
    $type = !empty($tag['type'])?$tag['type']: '';
    $order = isset($tag['order'])?$tag['order']:'listorder asc ,id desc';
    ...
    $data = M("Kefu")->where($where)->order($order)->select();

    if(empty($data))
      return  '';

    $site_name = $this->tpl->get('site_name');
    $site_wzqrcode = $this->tpl->get('site_wzqrcode');
      ...
      if($r['type']==1){
        ...
        foreach((array)$codes as $code){
          ...
          if($code){
            ...
            $datas.='<li class="qq"><a target="_blank" href="tencent://message/?uin='.$code.'" rel="nofollow">'.$r['name'].'</a></li>';
          }
        }
      }elseif($r['type']==2){
        ...
        foreach((array)$codes as $code){
          ...
          if($code){
            $codearr=explode("|",$code);
            $code= $codearr[0];$codename= $codearr[1];
            $datas.='<li class="wang"><a target="_blank" href="http://www.taobao.com/webww/ww.php?ver=3&touid='.$code.'&siteid=cntaobao&status='.$skin.'&charset=utf-8" rel="nofollow">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.$r['name'].'</a></li>';
          }
        }
      }elseif($r['type']==3){
        //spk
        ...
        foreach((array)$codes as $code){
          ...
          if($code){
            $codearr = explode("|",$code);
            $code = $codearr[0];$codename= $codearr[1];
            $datas.='<li class="spk"><a  target="_blank" href="skype:'.$code.'?chat" rel="nofollow">'.$r['name'].'</a></li>';
          }
        }
      }elseif($r['type']==4){
        //tel
        ...
        foreach((array)$codes as $code){
          if($code){
            $codearr=explode("|",$code);
            $code= $codearr[0];$codename= $codearr[1];
            $datas.='<li class="bs"></li><li class="tell"><a target="_blank" title="'.$code.'" rel="nofollow">'.$r['name'].'</a></li>';
          }
        }
      }else{
    ...
    $parsestr = '';
    $parsestr .= '<div class="kefu" id="'.$id.'"><div class="kefu-l"></div><div class="kefu-r"><div class="kftop"><div id="guanbi"></div></div><div class="kfbox"><ul>';
    $parsestr .= $datas;
    $parsestr .= '<li class="bs"></li><li class="codeer"><p>扫一扫</p><span id="codeer"><img src="'.$site_wzqrcode.'"/></span></li>';
  ...
  public function _list($tag,$content) {
    ...
    if ($tag['name'] || $tag['catid']) {   //根据用户输入的值拼接查询条件
      ...
      if(APP_LANG){
        $lang=$this->tpl->get('langid');
        if($tag['name']!='page' && $tag['name']!='winning'){ $where .= ' and lang='.$lang;}
      }
    ...
    //下面拼接输出语句
    $parsestr  = '';
    $parsestr .= '<?php  $_result='.$sql.'; if ($_result): $'.$key.'=0;$total=count($_result);';
    $parsestr .= 'foreach($_result as $key=>$'.$id.'):';
    $parsestr .= '++$'.$key.';$mod = ($'.$key.' % '.$mod.' );?>';
    $parsestr .= $content;//解析在article标签中的内容
    $parsestr .= '<?php endforeach; endif;?>';
