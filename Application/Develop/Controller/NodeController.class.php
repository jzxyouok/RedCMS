...
class NodeController extends AdminbaseController {
  ...
  function index(){
    ...
    foreach($this->groups as $key=>$res){
      ...
      $result = $this->db->where("groupid=$res[id]")->select();

      $array = array();
      foreach($result as $r) {
        $r['str_manage'] = '<a href="'.U('Node/add',array( 'pid' => $r['id'],'groupid'=>$r['groupid'])).'">'.L('add').'</a> |
        <a href="'.U('Node/edit',array( 'id' => $r['id'])).'">'.L('edit').'</a> |
        <a href="javascript:confirm_delete(\''.U('Node/delete',array( 'id' => $r['id'])).'\',\''.L('confirm',array('message'=>$r['cname'])).'\')">'.L('delete').'</a> ';
        $r['parentid']=$r['pid'];
  ...
  function _before_update(){
    if($_POST['pid']){
      $level =$this->db->getById($_POST['pid']);
