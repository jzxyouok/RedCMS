...
class ModuleController extends DevelopbaseController {
  ...
  function delete() {
    ...
    $tablename = C('DB_PREFIX').strtolower($this->module[$id]['name']);
    $m = $this->db->delete($id);
    if($m){
      $t =$this->db->execute("DROP TABLE IF EXISTS `".$tablename."`");
