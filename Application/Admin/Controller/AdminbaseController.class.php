...
class AdminbaseController extends Controller {
    ...
    if(APP_LANG){
      ...
      if(!empty($_GET['l'])){
        if($this->Lang[$_GET['l']]['id']){
          $_SESSION['ww_lang'] = $_GET['l'];
          $_SESSION['ww_langid'] = $this->Lang[$_GET['l']]['id'];
        }else{
          $this->error ( L ( 'NO_LANG' ) );
        }
      }elseif(!$_SESSION['ww_lang'] || !$_SESSION['ww_langid']){
        $_SESSION['ww_lang'] = $this->sysConfig['DEFAULT_LANG'];
        $_SESSION['ww_langid'] = $this->Lang[$this->sysConfig['DEFAULT_LANG']]['id'];
      }

      define('LANG_NAME',$_SESSION['ww_lang']);
      define('LANG_ID',$_SESSION['ww_langid']);
  ...
  public function create_show($id,$module) {
    ...
    if(APP_LANG){
      ...
      $T = F('config_'.LANG_NAME,'', './Themes/Home/'.$this->sysConfig['DEFAULT_THEME'].'/');
    }else{
    ...
    if($CONTENT_POS !== false){
      ...
      foreach ($pageurls as $p=>$urls) {
        ...
        $this->buildHtml($filename,$dir,'./Themes/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$template.C('TMPL_TEMPLATE_SUFFIX'));
      }
    }else{
    ...
    if($cat['moduleid']==1){
      ...
      if($CONTENT_POS !== false){
        ...
        foreach ($pageurls as $p=>$urls) {
          ...
          $r=$this->buildHtml($filename,$dir,'./Themes/Home/'.$this->sysConfig['DEFAULT_THEME'].'/page_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
        }
      }else{
        $this->assign($data);
      ...
      $r = $this->buildHtml($filename,$dir,'./Themes/Home/'.$this->sysConfig['DEFAULT_THEME'].'/'.$cat['module'].'_'.$template.C('TMPL_TEMPLATE_SUFFIX'));
    }
    if($r) return true;
  }
  ...
  public function create_index($sitemap) {
    ...
    if(empty($sitemap)){
      ...
      if(!$this->sysConfig['HOME_ISHTML']) $this->error(L('NO_HOME_ISHTML'));
      $this->assign('bcid',0);
      $r=$this->buildHtml('index','./'.$lang,'./Themes/Home/'.$this->sysConfig['DEFAULT_THEME'].'/Index_index'.C('TMPL_TEMPLATE_SUFFIX'));
    }else{
      $this->assign('sitemap','1');
      //$r=$this->buildHtml('sitemap','./'.$lang,'./Themes/Home/'.$this->sysConfig['DEFAULT_THEME'].'/sitemap'.C('TMPL_TEMPLATE_SUFFIX'));
      $r=$this->buildHtml('sitemap','./'.$lang,TMPL_PATH.'Home/'.$this->sysConfig['THEME'].'/Sitemap'.C('TMPL_TEMPLATE_SUFFIX'));
  ...
  function showMenu(){
    ...
    switch ($module_name) {
      ...
      case 'Link':
        ...
        break;
      case 'Kefu':
        $nav1 = 'operating';$nav2 = 1;
        break;
      case 'Feedback':
        $nav1 = 'operating';$nav2 = 2;
        break;
      case 'Resume':
        $nav1 = 'operating';$nav2 = 3;
        break;
      case 'Member':
        $nav1 = 'operating';$nav2 = 5;
        break;
      case 'Email':
        $nav1 = 'operating';$nav2 = 9;
        break;
      case 'Tags':
        $nav1 = 'operating';$nav2 = 10;
        break;
      default:
        $nav1 = 'dashboard';$nav2 = 1;
        break;
