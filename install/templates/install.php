...
//3. 读取数据库的sql文件(表结构)
$sql_content = file_get_contents("db/cms.sql");
echo "<p>读取数据局库配置文件！....</p>";
...
//7。添加后台管理员账户信息
$sql = "insert into `{$db_pre}admin` (id,role_id,username,password,email) values(1,1,'{$admin}','{$password}','{$mail}')";
...
if($is_data==1){
	...
	//导入必要的系统数据
	unset($sql_array);
	$sql_array=file('db/cms_sys_data.sql');
	foreach($sql_array as $v){
		...
		//替换表前缀
		$v=preg_replace("/INSERT INTO `fyt_(.*?)` VALUES/is","INSERT INTO `{$db_pre}\\1` VALUES",$v);
